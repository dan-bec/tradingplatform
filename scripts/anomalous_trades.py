...
con.execute("""
    ...
    JOIN trading_days td ON pd.data_date = td.data_date
""")

# Step 2: Filter for anomaly_category > 0, volume > anomaly_volume, and day_rank <= anomaly_category
con.execute("""
    CREATE OR REPLACE TABLE investigate_daily_data AS
    SELECT *
    FROM expanded_parsed_data
    WHERE anomaly_category > 0
      AND volume > anomaly_volume
      AND day_rank <= anomaly_category
""")

# Step 3: Filter for anomaly_category = 0 and volume >= anomaly_volume
con.execute("""
    CREATE OR REPLACE TABLE baseline_daily_data AS
    SELECT *
    FROM expanded_parsed_data
    WHERE anomaly_category = 0
      AND volume >= anomaly_volume
...
con.execute("""
    ...
    SELECT 
        ...
        idd.volume,
        idd.transactions,
        idd.anomaly_category,
        idd.anomaly_volume,
        idd.day_rank
    FROM trades_data td
    JOIN investigate_daily_data idd 
        ON td.ticker = idd.option_ticker AND td.data_date = idd.data_date
    LEFT JOIN option_condition_codes occ
        ON occ.id = td.conditions
    WHERE td.size > (idd.volume * 2.0 / idd.transactions)
    AND idd.transactions > 0
""")
...
large_trades_path = investigate_dir / f"_large_trades.csv"
con.execute(f"""
    COPY (
        SELECT * 
        FROM filtered_trades 
        WHERE trade_value > 100_000
