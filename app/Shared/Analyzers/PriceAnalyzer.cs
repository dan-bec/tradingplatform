...
namespace BullseyeDesktop.Shared.Analyzers
{
    ...
    public class PriceAnalyzer
    {
        ...
        public Action<string, string>? PriceAnalyzerMessageReceived;
        ...
        // 
        public PriceAnalyzer(string ticker, int totalLookbackSeconds) 
        {
            Ticker = ticker;
            _totalLookbackSeconds = totalLookbackSeconds;
            _analysisQueue = new ConcurrentQueue<PriceAnalysisEntry>();
            _start = DateTime.Now;
            _socket = new PolygonWebSocket(Ticker, this);

            _ = _socket.Main(); // purposely do not await and let it fire async
        }
        ...
        private PolygonWebSocket _socket;
